...
function VocabsPageContent() {
  ...
  const [vocabListCollapsed, setVocabListCollapsed] = useState(false);
  ...
  const filteredVocabs = useMemo(() => {
    ...
    let result = vocabs;
    if (selectedTagId) {
      result = result.filter((v) => v.vocabTags?.some((vt) => vt.tag.id === selectedTagId));
    }
    if (searchQuery.trim()) {
  ...
  const handleCreateTagFromSidebar = async () => {
    const name = newTagName.trim();
    if (!name) return;
    await handleCreateTag(name, newTagColor);
    setNewTagName("");
      ...
      {tagSidebarCollapsed ? (
        <div className="w-10 border-r border-line bg-panel flex flex-col shrink-0 items-center py-2 gap-1">
          ...
          <button
            ...
            className={`p-1.5 rounded transition-colors ${selectedTagId === null ? "text-ink bg-accent-soft" : "text-muted hover:text-ink hover:bg-highlight"}`}
            title="All Vocabs"
          >
          ...
          {tags.map((tag) => (
            <button
              ...
              className={`p-1.5 rounded transition-colors ${selectedTagId === tag.id ? "bg-accent-soft" : "hover:bg-highlight"}`}
              title={tag.name}
            >
              <span className="block w-3 h-3 rounded-full" style={{ backgroundColor: tag.color }} />
            </button>
          ))}
        </div>
      ) : (
        <div className="w-48 border-r border-line bg-panel flex flex-col shrink-0">
          ...
          <div className="flex-1 overflow-y-auto py-2">
            <button
              onClick={() => setSelectedTagId(null)}
              className={`w-full flex items-center gap-2 px-3 py-2 text-sm transition-colors ${selectedTagId === null ? "bg-accent-soft text-ink font-medium" : "text-muted hover:text-ink hover:bg-highlight"}`}
            >
            ...
            {tags.map((tag) => (
              <div key={tag.id} className="group relative flex items-center">
                <button
                  onClick={() => setSelectedTagId(tag.id)}
                  className={`flex-1 flex items-center gap-2 px-3 py-2 text-sm transition-colors ${selectedTagId === tag.id ? "bg-accent-soft text-ink font-medium" : "text-muted hover:text-ink hover:bg-highlight"}`}
                >
                  <span className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: tag.color }} />
                  <span className="flex-1 truncate text-left">{tag.name}</span>
                  {tag._count && <span className="text-xs text-muted">{tag._count.vocabTags}</span>}
                </button>
                <button
            ...
            {isCreatingTag && (
              <div className="px-2 py-2 space-y-2">
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") handleCreateTagFromSidebar();
                    if (e.key === "Escape") { setIsCreatingTag(false); setNewTagName(""); }
                  }}
                  className="w-full text-sm px-2 py-1 border border-line rounded-lg bg-surface text-ink focus:outline-none focus:border-accent placeholder:text-muted"
                />
                <div className="flex flex-wrap gap-1">
                  {PRESET_COLORS.map((c) => (
                    <button
                      ...
                      className={`w-5 h-5 rounded-full border-2 transition-all ${newTagColor === c ? "border-ink scale-110" : "border-transparent"}`}
                ...
                <div className="flex gap-1">
                  <button
                    onClick={handleCreateTagFromSidebar}
                  ...
                  <button
                    ...
                    className="p-1 text-muted hover:text-ink border border-line rounded-lg"
                  >
            ...
            {tags.length === 0 && !isCreatingTag && (
              <p className="px-3 py-2 text-xs text-muted">No tags yet. Click + to add one.</p>
            )}
          </div>
        </div>
      )}
      ...
      {vocabListCollapsed ? (
        ...
        <div className="w-64 border-r border-line bg-panel flex flex-col">
          <div className="px-5 py-4 border-b border-line bg-panel flex items-center justify-between">
            ...
            <button
              onClick={() => setVocabListCollapsed(true)}
              className="p-1.5 text-muted hover:text-ink hover:bg-highlight rounded-lg transition-colors"
              title="Collapse list"
            >
              <ChevronDoubleLeftIcon className="w-4 h-4" />
            </button>
            ...
            {vocabsLoading ? (
              ...
              searchQuery.trim() || selectedTagId ? (
                <EmptyState
                  ...
                  description={searchQuery.trim() ? `No vocabulary found matching "${searchQuery}"` : "No vocabulary with this tag"}
                  action={
                    searchQuery.trim()
